...
| `$block-diagram-drawer` | 画论文、报告或案例分析里的框图与方法示意图 | 以 figkit 构建脚本为源文件，输出 SVG 和 PNG；低饱和配色、字重与斜体层级，识别性物体用按需下载的 Tabler / LobeHub 开源 SVG 图标，用无头 Chrome 检查溢出、重叠、连线穿字、最小字号和覆盖率 |
