...
for k, (name, sym, tag) in enumerate((("view 1", "$x^{(1)}$", "front cam"), ("view 2", "$x^{(2)}$", "side cam"))):
    ...
    f.text(28, y + 19, name, size=13, weight=500, box=c)
    f.asset(HERE / "assets" / "camera.svg", 72, y + 7, 15, color=MUTED)
    f.text(184, y + 19, sym, size=14, anchor="end", box=c)
    f.scene(28, y + 27, 84, 60)
    f.chip(120, y + 31, 64, 20, "RGB", "gray", size=11.5, fill="#FFFFFF", family="mono")
    f.chip(120, y + 57, 64, 20, tag, "gray", size=11.5, fill="#FFFFFF", color=MUTED, family="serif", italic=True)
c = f.card(18, 266, 176, 96, fill="#FFFFFF", stroke=HAIR)
f.text(28, 285, "state", size=13, weight=500, box=c)
...
f.text(106, 352, "proprioception", size=11.5, color=MUTED, anchor="middle", box=c, family="serif", italic=True)
...
f.text(262, 81, "Encoder", size=13, weight=500, color=B.deep, box=c)
f.trapezoid(278, 94, 120, 46, "blue", "$E_\\theta$", size=20)
f.text(338, 158, "weights shared across views", size=11.5, color=MUTED, anchor="middle", box=c, family="serif", italic=True)
...
f.text(338, 340, "indexed latent batch", size=11.5, color=MUTED, anchor="middle", box=c, family="serif", italic=True)
...
f.text(484, 81, "Cross-view attention", size=13, weight=500, color=R.deep, box=c)
...
f.text(556, 272, "attention map · schematic", size=11.5, color=FAINT, anchor="middle", box=c, family="serif", italic=True)
...
f.text(698, 81, "Adaptive gate", size=13, weight=500, color=R.deep, box=c)
...
f.text(763, 218, "per-view gate weights", size=11.5, color=MUTED, anchor="middle", box=c, family="serif", italic=True)
...
f.chip(682, 310, 162, 24, "residual · layer norm", "gray", size=11.5, fill="#FFFFFF", family="mono")
...
f.text(898, 81, "Policy head", size=13, weight=500, color=G.deep, box=c)
...
f.text(1008, 192, "action chunk · schematic", size=11.5, color=FAINT, anchor="middle", box=c, family="serif", italic=True)

c = f.card(888, 214, 240, 146, "purple", stack=1)
f.text(898, 233, "World model", size=13, weight=500, color=P.deep, box=c)
...
f.text(1038, 256, "MSE", size=11, color=R.deep, anchor="middle", family="mono")
...
f.text(1148, 81, "Objective", size=13, weight=500, box=c)
...
f.text(1260, 342, "training loss · schematic", size=11.5, color=FAINT, anchor="middle", box=c, family="serif", italic=True)
