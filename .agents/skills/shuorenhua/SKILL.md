...
唯一例外是第 12 条补的英文术语全称。它解释的是原文已有的术语，不是新事实，不受这条限制。
...
这是作者的固定写作习惯，不是 AI 痕迹，也是本 skill 里唯一一条往文本里加东西的规则。加的只是原文已有术语的标准全称，帮读者接住术语，不引入新事实。

触发标记：同一篇里，英文缩写或专业术语第一次出现，且原文没有给全称。

改法：在首次出现处补一次全称，格式 `缩写 (Full Name)`，全称用英文原名，不硬翻成中文。之后再出现只用缩写，不重复补。

> ❌ 我们用 VLA 做端到端控制，VLA 的泛化比传统方案好。
>
> ✅ 我们用 VLA (Vision-Language-Action) 做端到端控制，VLA 的泛化比传统方案好。

> ❌ 规划层用的是 MPPI。
>
> ✅ 规划层用的是 MPPI (Model Predictive Path Integral)。

判断标准是“这篇的读者是否默认都认识”。面向机器人和具身智能圈时，`VLA`、`MPPI`、`HAL`、`SFT`、`BEV` 这类要补；面向更泛的科技读者时，`LLM`、`VLM`、`RL` 也要补。

不改：

- 读者烂熟的通用词不补，直接用：`AI`、`API`、`GPU`、`CPU`、`URL`、`HTTP`、`JSON`、`SDK`、`CLI`、`OS`、`PDF`
- 原文已经写了全称，或已有 `缩写（全称）` 结构的，不重复补，也不改成另一种写法
- 不确定标准全称是什么时，不补。宁可漏补，不可编造
- 代码块、命令、路径、引语、小标题内部不补；小标题里首次出现的术语，到正文首次出现处再补
- 人名、产品名、公司名这类专名不按术语处理
...
- 改写后每个实词能否在原文指出出处。姓名、数字、日期、引语、来源、因果，任何指不出出处的都必须撤销。第 12 条补的术语全称除外
- 专业英文缩写首次出现是否补了全称，后续有没有重复补；免补清单里的通用词有没有被啰嗦地补全称；有没有补了自己不确定的全称
